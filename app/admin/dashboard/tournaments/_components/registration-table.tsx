...
export function RegistrationTable({
  ...
  const renderCell = (item: any, key: string) => {
    ...
    if (type === "TEAM") {
      ...
      if (key === "player1")
        return `${item.player1Name} (${item.player1Email})`;
      if (key === "player2")
        return `${item.player2Name} (${item.player2Email})`;
    }
  ...
  return (
    <div className="space-y-4 border-t border-[#2e3a28]/10 pt-6">
      <div className="flex items-center justify-between">
        <h2 className="font-garamond text-lg font-bold text-[#2e3a28]">
          {title}
        </h2>
        {exportUrl && (
          <a
          ...
          <tbody>
            {data.length === 0 ? (
              ...
              data.map((item) => (
                <tr
                  key={item.id}
                  className="border-b border-[#2e3a28]/5 hover:bg-[#2e3a28]/5"
                >
                  {columns.map((col, i) => (
                  ...
                  <td className="p-2 flex gap-2">
                    <Link
                      href={`/admin/dashboard/registrations/${item.id}/edit`}
                    ...
                    <form
                      action={async () => {
                        await deleteRegistrationAction(
                          item.id,
                          type,
                          tournamentId,
                        );
