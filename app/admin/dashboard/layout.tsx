...
export default async function AdminDashboardLayout({ children }: { children: React.ReactNode }) {
  ...
  return (
    <div className="min-h-screen bg-[#fcfbf9] flex flex-col lg:flex-row">
      <aside className="w-full lg:w-56 shrink-0 border-b lg:border-b-0 lg:border-r border-[#2e3a28]/10 bg-white p-4 lg:p-6 space-y-6">
        <div>
          <span className="font-garamond text-lg font-bold text-[#2e3a28]">UNDS Admin</span>
        </div>
        <nav className="space-y-1">
          {NAV_ITEMS.map((item) => (
            <Link
              key={item.href}
              href={item.href}
              className="flex items-center justify-between px-3 py-2 rounded-xs text-xs font-ui tracking-wider text-text-secondary hover:bg-[#2e3a28]/5 hover:text-[#2e3a28] transition-colors"
            >
              <span>{item.label}</span>
              <span className="text-[10px] font-bold bg-[#2e3a28]/10 text-[#2e3a28] px-1.5 py-0.5 rounded-xs">
                {counts[item.label]}
              </span>
            </Link>
          ))}
        ...
        <form action={logoutAction} className="pt-4 border-t border-[#2e3a28]/10">
          <button type="submit" className="text-xs font-ui tracking-wider text-red-700 cursor-pointer">
            Sign Out
          </button>
        </form>
      </aside>
