...
export function PSTeamForm({
  state,
  action,
  isPending,
  event,
}: BaseFormProps) {
  const isLocked = event.psRegistrationLocked;
      ...
      <div className="w-full bg-[color-mix(in_srgb,var(--surface)_95%,black)] border border-[#2e3a28] p-8 rounded-xs flex flex-col items-center text-center gap-4 my-6">
        ...
        <div className="space-y-1">
          <h3 className="font-garamond text-xl font-bold text-text-primary">
            Public Speaking Registration Closed
          </h3>
          <p className="font-garamond text-sm text-text-secondary max-w-md leading-relaxed">
            Public Speaking participant registration is currently closed. Please contact the tournament organizers or the adjudication desk if you have inquiries.
          </p>
        </div>
        <div className="pt-2">
          <a
