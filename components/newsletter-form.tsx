...
export function NewsletterForm({ source, className = "" }: NewsletterFormProps) {
  ...
  if (state.success) {
    return (
      <p className={`text-xs font-manrope text-[#2e3a28] font-bold ${className}`}>
    ...
    <div className={className}>
      ...
      >
        ...
        <button
          type="submit"
          disabled={isPending}
          className="px-5 btn btn-accent rounded-xs! text-xs  tracking-wider font-bold transition-colors hover:bg-[#1f281b] disabled:opacity-50"
        >
          {isPending ? "..." : "Subscribe"}
        </button>
      </form>
      {state.error && <p className="text-xs font-manrope text-red-700 mt-2 text-center">{state.error}</p>}
    </div>
  );
}
